...
public sealed class MainWindowViewModel : ObservableObject
{
    ...
    public void AddObservation(HookObservation observation)
    {
        ...
        if (observation.GenerationId is null || observation.IsSessionLifecycle || observation.IsTabHook)
        {
        ...
        TreeNodeViewModel generationNode = GetOrCreateGenerationNode(observation, sessionNode);
    ...
    private TreeNodeViewModel ResolveSessionNode(HookObservation observation)
    {
        if (observation.SessionId is not null &&
            _sessionNodes.TryGetValue(observation.SessionId, out TreeNodeViewModel? existingSessionNode))
        {
        ...
        return GetOrCreateSessionNode(sessionId, workspaceNode, observation.SessionId is null);
    }
    ...
    private TreeNodeViewModel GetOrCreateGenerationNode(HookObservation observation, TreeNodeViewModel sessionNode)
    {
        ...
        string generationKey = $"{sessionNode.Key}\0{observation.GenerationId}";
        ...
        bool isUnknownSession)
    {
        string sessionKey = isUnknownSession
            ? $"{workspaceNode.Key}\0{UnknownSession}"
            : sessionId;
